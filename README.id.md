...
1. **Dapatkan Token**: Lakukan request `POST` ke `/login` menggunakan data akun default (dari _seeder_):
    - **Email**: `admin@gmail.com`
    - **Password**: `admin@gmail.com`
    
2. **Gunakan Token**: Salin token dari respons. Untuk mengakses _endpoint_ yang dilindungi, tambahkan _header_ `Authorization` pada request Anda:
    HTTP
    ```
    Authorization: Bearer <token_anda>
    ```
3. **Akses Swagger**: Buka `http://localhost:8080/swagger/index.html`. Klik tombol **Authorize**, masukkan token Anda, dan silakan mencoba semua _endpoint_ secara interaktif.
